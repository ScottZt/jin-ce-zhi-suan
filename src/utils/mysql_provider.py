...
class MysqlProvider:
    def __init__(self, host=None, port=None, user=None, password=None, database=None, charset=None):
        ...
        self.host = str(host or cfg.get("data_provider.mysql_host", "127.0.0.1")).strip() or "127.0.0.1"
        self.port = int(port or cfg.get("data_provider.mysql_port", 3306) or 3306)
        self.user = str(user or cfg.get("data_provider.mysql_user", "")).strip()
        self.password = str(password or cfg.get("data_provider.mysql_password", "")).strip()
        self.database = str(database or cfg.get("data_provider.mysql_database", "")).strip()
        self.charset = str(charset or cfg.get("data_provider.mysql_charset", "utf8mb4")).strip() or "utf8mb4"
        self.pool_size = max(1, int(cfg.get("data_provider.mysql_pool_size", 6) or 6))
        self.pool_wait_timeout_sec = max(1.0, float(cfg.get("data_provider.mysql_pool_wait_timeout_sec", 10) or 10))
        self.page_size = max(1000, int(cfg.get("data_provider.mysql_query_page_size", 20000) or 20000))
        self.last_error = ""
    ...
    def _load_pymysql(self):
        try:
            ...
            self.last_error = f"pymysql 未安装或导入失败: {e}"
            return None
    ...
    def _create_connection(self):
        ...
        try:
            ...
            self.last_error = f"MySQL 连接失败: {e}"
            return None
    ...
    def _acquire_connection(self):
        with self._pool_cond:
            while True:
                ...
                if not waited:
                    self.last_error = f"MySQL 连接池等待超时（{self.pool_wait_timeout_sec}s）"
                    return None
    ...
    def _resolve_table_name(self, interval):
        cfg = ConfigLoader.reload()
        key_map = {
            "1min": "data_provider.mysql_table_1min",
            "5min": "data_provider.mysql_table_5min",
            "10min": "data_provider.mysql_table_10min",
            "15min": "data_provider.mysql_table_15min",
            "30min": "data_provider.mysql_table_30min",
            "60min": "data_provider.mysql_table_60min",
            "D": "data_provider.mysql_table_day",
        }
    ...
    def upsert_kline_data(self, df, interval="1min", batch_size=2000):
        ...
        if not table:
            self.last_error = f"未配置 {interval} 对应的 MySQL 表名"
            return 0
        ...
        written = 0
        sql = (
            f"INSERT INTO `{table}` (code, trade_time, open, high, low, close, vol, amount) "
            f"VALUES (%s, %s, %s, %s, %s, %s, %s, %s) "
            f"ON DUPLICATE KEY UPDATE "
            f"open=VALUES(open), high=VALUES(high), low=VALUES(low), close=VALUES(close), "
            f"vol=VALUES(vol), amount=VALUES(amount)"
        )
        try:
            ...
            if "duplicate entry" in err_text:
                self.last_error = (
                    f"MySQL 写入缓存失败: {e}。"
                    f"请确认唯一键为 (code, trade_time) 或按业务调整唯一约束"
                )
            else:
                self.last_error = f"MySQL 写入缓存失败: {e}"
            return 0
        finally:
    ...
    def _query_range(self, code, start_time, end_time, interval):
        ...
        try:
            ...
            broken = True
            self.last_error = f"MySQL 查询失败: {e}"
            return pd.DataFrame()
        finally:
            self._release_connection(conn, broken=broken)
    ...
    def check_connectivity(self, code):
        conn = self._acquire_connection()
        if conn is None:
            return False, self.last_error or "MySQL 连接失败"
        broken = False
        try:
            with conn.cursor() as cursor:
            ...
            broken = True
            self.last_error = f"MySQL 连通性检查失败: {e}"
    ...
    def get_latest_bar(self, code):
        table = self._resolve_table_name("1min")
        if not table:
            self.last_error = "未配置 mysql_table_1min"
            return None
        conn = self._acquire_connection()
        if conn is None:
            return None
        ...
        sql = (
            f"SELECT code, trade_time AS dt, open, high, low, close, vol, amount "
            f"FROM `{table}` WHERE code = %s ORDER BY trade_time DESC LIMIT 1"
        )
        try:
            for c in self._code_variants(code):
                with conn.cursor() as cursor:
            ...
            broken = True
            self.last_error = f"MySQL latest 查询失败: {e}"
            return None
        finally:
            self._release_connection(conn, broken=broken)
        return None
